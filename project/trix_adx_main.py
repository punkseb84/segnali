...
def main() -> None:
    ...
    adx_threshold = float(os.getenv("TRIX_ADX_THRESHOLD", "18"))
    volume_ratio_min = float(os.getenv("TRIX_VOLUME_RATIO_MIN", "0.70"))
    max_extension_atr = float(os.getenv("TRIX_MAX_EXTENSION_ATR", "1.50"))
    min_stop_atr = float(os.getenv("TRIX_MIN_STOP_ATR", "1.0"))
    max_stop_atr = float(os.getenv("TRIX_MAX_STOP_ATR", "1.5"))
    trailing_atr = float(os.getenv("TRIX_TRAILING_ATR", "1.8"))
    be_buffer = float(os.getenv("TRIX_BREAK_EVEN_BUFFER_RATE", "0.0002"))
    min_score = float(os.getenv("TRIX_MIN_SCORE", "58"))
    max_per_cycle = max(1, int(os.getenv("TRIX_MAX_PER_CYCLE", "3")))
    max_per_day = max(1, int(os.getenv("TRIX_MAX_PER_DAY", "12")))
    max_open_positions = max(1, int(os.getenv("TRIX_MAX_OPEN_POSITIONS", "4")))
    max_daily_stops = max(1, int(os.getenv("TRIX_MAX_DAILY_STOPS", "4")))
