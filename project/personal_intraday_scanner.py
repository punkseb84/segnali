"""Private daily spot selector with transparent ranking and robust volume."""
...
from project.relative_strength_v5_scanner import RelativeStrengthV5Scanner
...
RUNTIME_VERSION = "PRIVATE_BEST_OPPORTUNITY_12H_V3"
ROME = ZoneInfo("Europe/Rome")


class PersonalIntradayScanner(RelativeStrengthV5Scanner):
    def __init__(
        self,
        *args: Any,
    ...
    def _publish_decision(
        ...
        message = (
            ...
            f"{lines}\n\n"
            "Volume: ultime quattro candele 15m confrontate con la mediana delle finestre 1h precedenti."
        )
    ...
    def evaluate(self) -> GeneratedSignal | None:
        ...
        for pair in self.pairs:
            ...
            try:
                ...
                self.logger.exception("PRIVATE_V3 pair_failed pair=%s", pair)
                continue
            if item is not None:
        ...
        ranked.sort(
            key=lambda value: (
                float(value["quality_score"]),
                float(value["rs_score"]),
            ),
            reverse=True,
        )
        ...
        for item in ranked[:5]:
            ...
            validity = "ok" if item.get("volume_data_valid") else "fallback neutro"
            diagnostics.append(
                f"<b>{item['pair']}</b>: qualità {float(item['quality_score']):.0f}/100, "
                f"RS {float(item['rs_score']):+.2f}, vol1h {float(item['volume_ratio']):.2f}x "
                f"(15m {float(item.get('volume_ratio_15m') or 0.0):.2f}x, {validity}) — "
        ...
        for rank, item in enumerate(ranked[:8], start=1):
            ...
            breakdown.update(
                {
                    ...
                    "max_hold_hours": 12,
                    "volume_method": "ROBUST_ONE_HOUR_WINDOW",
            ...
            candidate = replace(
                ...
                reasons=[
                    *candidate.reasons,
                    f"migliore opportunità giornaliera qualità {quality:.0f}/100",
                    "uscita obbligatoria entro 12 ore",
                ],
            )
            ...
            self.logger.info(
                "PRIVATE_V3_BUY id=%s pair=%s quality=%.1f budget=%.2f volume1h=%.3f",
                signal_id,
                signal.pair,
                quality,
                budget,
                float(item.get("volume_ratio_1h") or 0.0),
            )
            return signal

        best_quality = float(ranked[0]["quality_score"]) if ranked else 0.0
        self._publish_decision(
            f"migliore qualità {best_quality:.0f}/100, soglia {self.min_quality_score:.0f}/100",
            ranked,
            diagnostics,
        )
        return None
