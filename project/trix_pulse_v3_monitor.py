"""Direction-aware PAPER management for TRIX Pulse V3 LONG/SHORT positions."""
...
from project.trix_pulse_v3_scanner import RUNTIME_VERSION


class TrixPulseV3Monitor(TrixAdxV2Monitor):
    """Protect at +0.8R, take 50% at +1.5R, trail the remaining leg at 1.8 ATR."""
    ...
    def _arm_break_even(self, signal: dict[str, Any], state: dict[str, Any], timestamp: Any) -> None:
        ...
        trigger = entry + 0.8 * risk if direction == "LONG" else entry - 0.8 * risk
        ...
        self.event_bus.publish(Event(EventType.REPORT_READY, {
            "message": f"🟡 <b>PROTEZIONE +0,8R · PAPER</b> <code>#{signal['id']}</code>\n<b>{signal['pair']}</b> · {direction} · 15m\nStop al pareggio netto: <b>{break_even:.8f}</b>",
            "trusted_html": True, "signal_id": signal["id"],
        }))
    ...
    def check_signal(self, signal: dict[str, Any]) -> bool:
        ...
        for index in range(1, len(data)):
            ...
            be_trigger = entry + 0.8 * risk if direction == "LONG" else entry - 0.8 * risk
            ...
            trix_signal = float(latest["trix_signal"]) if not pd.isna(latest["trix_signal"]) else trix_now
            trix_prev = float(previous["trix"]) if not pd.isna(previous["trix"]) else trix_now
            trix_signal_prev = float(previous["trix_signal"]) if not pd.isna(previous["trix_signal"]) else trix_signal
            ema50 = float(latest["ema50"])
            opposite_cross = (trix_prev >= trix_signal_prev and trix_now < trix_signal) if direction == "LONG" else (trix_prev <= trix_signal_prev and trix_now > trix_signal)
            ema_exit = close < ema50 if direction == "LONG" else close > ema50
            if opposite_cross or ema_exit:
                return self._close_signal(signal, outcome_price=close, timestamp=timestamp, open_price=op, high_price=high, low_price=low, close_price=close, resolution="TRIX_PULSE_DIRECTIONAL_EXIT", state=state)
            ...
            if state.get("tp1_hit"):
                ...
                if direction == "LONG":
                    ...
                    new_stop = max(stop, be, extreme - atr * self.trailing_atr_multiple)
                    patch["highest_high_since_tp1"] = extreme
                    improve = new_stop > stop
                else:
                    extreme = min(float(state.get("lowest_low_since_tp1") or low), low)
                    new_stop = min(stop, be, extreme + atr * self.trailing_atr_multiple)
