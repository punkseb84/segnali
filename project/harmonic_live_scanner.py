...
class HarmonicLiveStrategyScanner(OperationalLongStrategyScanner):
    ...
    def harmonic_assessment(
        ...
        if pattern is None:
            return cls._assessment(
                HARMONIC_STRATEGY, 0, False, [],
                ["nessun pattern armonico bullish completato"], 1.80
            )
        ...
        result = cls._assessment(
            HARMONIC_STRATEGY, score,
            count >= 3 and not_chasing and context_safe and target_room and score >= 78,
            [f"pattern bullish {pattern['pattern']} completato", "punto D confermato su candele chiuse", f"conferme={count}/4"],
            blockers, 1.80,
        )
        result.update({
    ...
    def evaluate_pair_detailed(self, pair: str) -> tuple[GeneratedSignal | None, str, dict[str, Any]]:
        f15 = self.fetch_closed_frame(pair, "15m", 260)
        f1h = self.fetch_closed_frame(pair, "1h", 260)
        if len(f15) < 210 or len(f1h) < 210:
            return None, "INSUFFICIENT_OHLC", {"pair": pair, "regime": "UNKNOWN", "strategy": "NONE", "score": 0.0, "blockers": [f"storico insufficiente 15m={len(f15)} 1h={len(f1h)}"]}
        ...
        ordered = sorted(assessments, key=lambda item: self.assessment_priority(item, self._strategy_states), reverse=True)
        fallback, last_failure = ordered[0], None
        for assessment in ordered:
            if not assessment.get("qualified"):
                continue
            signal, reason, diagnostic = self._build_signal_for_assessment(pair, f15, regime, relative, assessment)
        ...
        return None, f"NO_SETUP_{regime}", {
            "pair": pair, "regime": regime, "strategy": strategy,
            "strategy_state": str(self._strategy_states.get(strategy, {}).get("state") or "ACTIVE"),
            "score": float(fallback.get("score") or 0), "qualified": False,
    ...
    def fetch_loss_streak(self) -> tuple[int, datetime | None]:
        ...
        rows = self.client.fetch_all(
            ...
            ORDER BY closed_at DESC, id DESC LIMIT 12""", LIVE_LONG_STRATEGIES
        )
        streak, last_closed = 0, None
        for status, closed_at in rows:
            if last_closed is None and isinstance(closed_at, datetime):
                last_closed = closed_at if closed_at.tzinfo else closed_at.replace(tzinfo=timezone.utc)
            if str(status) != "STOP_LOSS":
                break
            streak += 1
        return streak, last_closed

    def stop_distance_for_strategy(self, strategy: str, frame: pd.DataFrame, entry: float, atr: float) -> float:
        if atr <= 0:
            return 0.0
        if strategy == HARMONIC_STRATEGY:
            ...
            floors = {"Trend Pullback": 1.45, "Breakout 20": 1.35, "Range Bounce Bollinger": 1.35, "Oversold Reversal": 1.60, "Relative Strength Momentum": 1.45, "Volatility Squeeze Breakout": 1.40}
            distance = max(distance, atr * floors.get(strategy, 1.45))
        return min(max(distance, entry * 0.004), entry * 0.025)

    def technical_target(self, strategy: str, frame: pd.DataFrame, entry: float, stop_distance: float, atr: float, target_rr: float) -> float:
        if strategy != HARMONIC_STRATEGY:
            return super().technical_target(strategy, frame, entry, stop_distance, atr, target_rr)
        target = float((self._building_assessment or {}).get("harmonic_target", entry + stop_distance * target_rr))
        return max(entry + stop_distance * 1.30, min(target, entry * 1.03, entry + atr * 3.0))
    ...
    def calculate_net_economics(self, entry: float, stop_loss: float, take_profit: float) -> dict[str, float | bool]:
        ...
        if loss <= self.max_net_loss_eur or original <= self.min_notional_eur:
            return result
        adjusted_notional = max(self.min_notional_eur, original * self.max_net_loss_eur / max(loss, 1e-9) * 0.98)
        try:
    ...
    def _build_signal_for_assessment(self, pair: str, frame_15m: pd.DataFrame, regime: str, relative: dict[str, float], assessment: dict[str, Any]) -> tuple[GeneratedSignal | None, str, dict[str, Any]]:
        strategy, latest = str(assessment.get("strategy") or ""), frame_15m.iloc[-1]
        atr = float(latest["atr"])
        if strategy != HARMONIC_STRATEGY and atr > 0 and float(latest["close"]) > float(latest["ema20"]) + atr * 1.80:
            return None, "ENTRY_TOO_EXTENDED", {"pair": pair, "regime": regime, "strategy": strategy, "score": float(assessment.get("score") or 0), "qualified": True, "blockers": ["ingresso troppo distante da EMA20 rispetto all'ATR"], "rsi": round(float(latest["rsi"]), 1), "volume_ratio": round(float(latest["volume_ratio"]), 2), "candle_time": str(latest["timestamp"])}
        if self._loss_guard_active and strategy != HARMONIC_STRATEGY and assessment.get("confirmation_mode") == "CORE_PLUS_CONFIRMATIONS":
            return None, "LOSS_STREAK_STRICT_MODE", {"pair": pair, "regime": regime, "strategy": strategy, "score": float(assessment.get("score") or 0), "qualified": True, "blockers": [f"setup adattivo sospeso dopo {self._loss_streak} stop consecutivi"], "rsi": round(float(latest["rsi"]), 1), "volume_ratio": round(float(latest["volume_ratio"]), 2), "candle_time": str(latest["timestamp"])}
        self._building_assessment = assessment
        try:
            signal, reason, diagnostic = super()._build_signal_for_assessment(pair, frame_15m, regime, relative, assessment)
        finally:
        ...
        if signal.entry_notional_eur < self.trade_notional_eur * 0.99:
            breakdown.update({"risk_sized": True, "max_net_loss_eur": round(self.max_net_loss_eur, 2)})
            reasons.append(f"posizione ridotta per limitare la perdita netta a circa €{self.max_net_loss_eur:.2f}")
        if strategy == HARMONIC_STRATEGY:
            breakdown.update({"harmonic_live": True, "harmonic_quality": float(assessment.get("harmonic_quality") or 0)})
    ...
    def evaluate(self) -> GeneratedSignal | None:
        ...
        self._loss_guard_active = bool(streak >= self.loss_streak_trigger and last_closed and datetime.now(timezone.utc) < last_closed + timedelta(hours=self.loss_guard_hours))
        original = (self.max_signals_per_cycle, self.min_signal_score, self.min_net_rr, self.min_tp1_net_profit_eur)
        ...
        try:
            ...
            self.max_signals_per_cycle, self.min_signal_score, self.min_net_rr, self.min_tp1_net_profit_eur = original
        self._last_cycle_summary.update({"live_strategies": 7, "loss_streak": streak, "loss_guard_active": self._loss_guard_active, "max_net_loss_eur": self.max_net_loss_eur})
        self.logger.info("HARMONIC_LIVE_V5 strategies=7 loss_streak=%s loss_guard=%s max_net_loss_eur=%.2f", streak, self._loss_guard_active, self.max_net_loss_eur)
        return result
