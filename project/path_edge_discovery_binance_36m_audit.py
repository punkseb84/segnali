...
This study is intentionally separated from the already-observed 2024-2026
research.  TP/SL/max-hold selection is performed ONLY on 2023-08-09 ->
2024-02-09.  At most one frozen configuration is selected per event x timeframe
family.  That frozen configuration must then survive chronologically:

- DEV:        2023-08-09 -> 2024-02-09 (selection allowed)
- VALIDATION: 2024-02-09 -> 2024-05-09 (no parameter changes)
- HOLDOUT:    2024-05-09 -> 2024-08-09 (no parameter changes)
- EXTERNAL A: 2024-08-09 -> 2025-08-09 (already-observed later year)
- EXTERNAL B: 2025-08-09 -> 2026-08-09 (final replication)

The event definitions are the same 18 predeclared events from Event Move
Discovery.  Timeframes are 15m/30m/1h/2h/4h.  Entry is the next-bar open after
the event is known on a closed bar.  Exit is whichever happens first:
predeclared target, predeclared stop, or the frozen maximum holding horizon.
Same-bar ambiguity is conservative: stop before target.

The barrier grid is fixed before results.  It is deliberately small and
structural rather than continuously optimized.  Costs use the configured taker
fee each side plus configured spread/slippage.
"""
...
AUDIT_VERSION = "PATH_EDGE_DISCOVERY_BINANCE_36M_20260809_V1"
...
# Eight economically distinct target/stop structures, fixed in advance.
...
MIN_EXTERNAL = 25
DEV_MIN_PF = 1.15
DEV_MIN_EXP = 0.0010       # +0.10% net/trade after costs
FINAL_MIN_PF = 1.10
FINAL_MIN_EXP = 0.0005     # +0.05% net/trade after costs
...
class PathEdgeDiscoveryBinance36mAudit(EventMoveDiscoveryBinance24mAudit):
    ...
    def _fetch_symbol(self, symbol: str) -> tuple[pd.DataFrame, dict[str, Any]]:
        ...
        session.headers.update({"User-Agent": "path-edge-discovery/1.0"})
    ...
    def _resample36(raw: pd.DataFrame, tf: str) -> pd.DataFrame:
        ...
        mins = TF_MINUTES[tf]
        return x[(x["timestamp"] + pd.Timedelta(minutes=mins)) <= EXTERNAL_B_END].reset_index(drop=True)

    def _prepare36(self, raw: pd.DataFrame, tf: str) -> tuple[pd.DataFrame, dict[str, tuple[pd.Series, str]]]:
        # Reproduce exactly the previous 18 event definitions, but on the longer history.
    ...
    def _simulate_pair_candidate(
        ...
        for signal_i in indices:
            ...
            trades.append({
                ...
                "exit_time": pd.Timestamp(x.iloc[exit_i]["timestamp"]),
                "phase": phase,
                "gross_r": gross,
                "fee_r": fee_only,
                "net_r": net,
    ...
    def run(self) -> dict[str, Any]:
        ...
        # Selection is isolated to DEV: choose at most one barrier/hold config per
        # event x timeframe family. Later phases never choose among alternatives.
        for tf in TIMEFRAMES:
            for event_name in event_names:
                ...
                for barrier_name, tp_pct, sl_pct in BARRIERS:
                    for hold_h in MAX_HOLDS_H:
                        ...
                        for pair in PAIR_NAMES.values():
                            ...
                            all_rows.extend(self._simulate_pair_candidate(
                                x, mask, direction, pair, tf, event_name, tp_pct, sl_pct, hold_h
                            ))
                        dev_rows = self._phase_rows(all_rows, "DEV")
                ...
                dev_ci = self._bootstrap(best["rows"] and self._phase_rows(best["rows"], "DEV"), seed=310000 + len(family_rows))
                best["dev_ci"] = dev_ci
                best["dev_pass"] = bool(
                    best["dev"]["n"] >= MIN_DEV
                    and best["dev"]["pf"] >= DEV_MIN_PF
                    and best["dev"]["exp"] >= DEV_MIN_EXP
                    and dev_ci[0] > 0.0
                )
                if best["dev_pass"]:
                    dev_qualified += 1

                val = self._stats(self._phase_rows(best["rows"], "VAL"))
                holdout = self._stats(self._phase_rows(best["rows"], "HOLDOUT"))
                ext_a = self._stats(self._phase_rows(best["rows"], "EXTERNAL_A"))
                ext_b_rows = self._phase_rows(best["rows"], "EXTERNAL_B")
                ext_b = self._stats(ext_b_rows)
                best.update({"val": val, "holdout": holdout, "external_a": ext_a, "external_b": ext_b})

                best["val_pass"] = bool(
                    best["dev_pass"] and val["n"] >= MIN_VAL and val["pf"] > 1.0 and val["exp"] > 0.0
                )
                if best["val_pass"]:
                    passed_val += 1
                ...
                best["confirmed"] = bool(
                    best["external_a_pass"]
                    and ext_b["n"] >= MIN_EXTERNAL
                    and ext_b["pf"] >= FINAL_MIN_PF
                    and ext_b["exp"] >= FINAL_MIN_EXP
                    and ext_b_ci[0] > 0.0
                    and ext_b["top_pair_share"] <= 0.70
                )
                if best["confirmed"]:
                    confirmed.append(best)

                family_rows.append({k: v for k, v in best.items() if k != "rows"})

        top_dev = sorted(
            family_rows,
            key=lambda r: (float(r["dev"]["exp"]), float(r["dev"]["pf"])),
            reverse=True,
        )[:8]
        finalists = sorted(
            [r for r in family_rows if r.get("external_a_pass")],
            key=lambda r: (float(r["external_b"]["exp"]), float(r["external_b"]["pf"])),
            reverse=True,
        )[:8]
        confirmed_clean = [{k: v for k, v in r.items() if k != "rows"} for r in confirmed]
        ...
        summary = {
            ...
            "passed_external_a": passed_external_a,
            "confirmed": confirmed_clean,
            "top_dev": top_dev,
            "finalists": finalists,
            "coverage": coverage,
    ...
    def format_report(s: dict[str, Any]) -> str:
        ...
        lines = [
            ...
            "Domanda: dopo un evento, TP o SL viene raggiunto prima con expectancy netta replicabile?",
            f"Eventi: <b>{s.get('events',0)}</b> · TF: 15m/30m/1h/2h/4h · griglia grezza: <b>{s.get('raw_grid',0)}</b>",
            "Per ogni evento×TF, una sola configurazione TP/SL/hold viene scelta sul DEV e poi congelata.",
        ...
        if s.get("finalists"):
            ...
            for r in s["finalists"][:6]:
                a = r["external_a"]
                b = r["external_b"]
                ci = r.get("external_b_ci", (0.0, 0.0))
                lines.append(
                    f"• {r['tf']} · {r['event']} · TP {100*r['tp_pct']:.2f}% / SL {100*r['sl_pct']:.2f}% / {r['hold_h']}h · "
        ...
        for c in s.get("coverage", []):
            lines.append(
                f"• {c['pair']}: {c['bars']} barre · gap {c['missing_intervals']} · richieste {c['requests']}"
            )
        if s.get("confirmed"):
            lines += [
                "",
                "✅ <b>PATH EDGE REPLICATO</b>: almeno una configurazione scelta nel 2023–2024 sopravvive ai due anni esterni con i gate congelati.",
                "Prima di qualsiasi live serve comunque un audit portfolio/esecuzione separato.",
            ]
        else:
            lines += [
                "",
                "❌ <b>NESSUN PATH EDGE REPLICATO</b> con eventi, barriere e costi predefiniti.",
                "Nessun parametro viene ritoccato usando VALIDATION, HOLDOUT o i due anni esterni.",
            ]
        lines.append("⚠️ Gli eventi SHORT sono teorici su candele Spot; operatività short reale richiede margin/futures.")
        return "\n".join(lines)
