...
PUBLIC_BUDGET_EUR=100.0;MAX_OPEN=4;PER_TRADE_EUR=25.0;BINANCE_VIP1_MAKER_FEE_RATE=.0007;BINANCE_VIP1_TAKER_FEE_RATE=.0016

def _retire_previous_open_signals(postgres:object)->None:
    postgres.execute("""UPDATE signals.generated_signals SET status='EXPIRED', closed_at=COALESCE(closed_at,NOW()), outcome_resolution='REPLACED_BY_VELEZ_MODE2_PROTECT_RUNTIME' WHERE status IN ('NEW','OPEN') AND COALESCE(score_breakdown->>'runtime_version','') <> %s""",(RUNTIME_VERSION,))

def main()->None:
    pairs=resolve_top_market_cap_pairs(20);settings=apply_simple_policy(load_settings());settings=replace(settings,collector_pairs=pairs,collector_timeframes=['15m'],operational_timeframe='15m',scheduler_collector_seconds=900,telegram_send_startup_message=False,enable_daily_signal_report=False,binance_buy_fee_rate=BINANCE_VIP1_TAKER_FEE_RATE,binance_sell_fee_rate=BINANCE_VIP1_TAKER_FEE_RATE)
    logger=get_module_logger('system');logger.info('======================================');logger.info('PROJECT MAIN: VELEZ 15M MODE 2 PROTECTED PAPER ONLY');logger.info('======================================');logger.info('VELEZ_RUNTIME version=%s timeframe=15m pairs=%s budget=%.2f per_trade=%.2f fixed_tp=false protection=1.5R_to_1R exit=protected_stop_or_ema20_close_or_12h relative_strength=false trix=false ichimoku=false paper=true notifier=strict',RUNTIME_VERSION,pairs,PUBLIC_BUDGET_EUR,PER_TRADE_EUR)
    ...
    except Exception as exc:logger.error('PostgreSQL connection failed: %s',sanitize_postgres_error(str(exc),settings.database_url));raise SystemExit(1) from exc
    logger.info('PostgreSQL connection: OK');logger.info('PostgreSQL %s',parse_postgres_connection_info(settings.database_url).display());run_migrations(postgres);run_capital_protection_migration(postgres);monitor_module.RUNTIME_VERSION=RUNTIME_VERSION
