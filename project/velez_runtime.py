...
def main() -> None:
    pairs=resolve_top_market_cap_pairs(20); settings=apply_simple_policy(load_settings())
    ...
    logger=get_module_logger("system"); logger.info("======================================"); logger.info("PROJECT MAIN: VELEZ 15M MODE 2 PROTECTED PAPER ONLY"); logger.info("======================================")
    ...
    # Historical Velez audits are retained but one-shot markers make them no-op after completion.
    ...
    # New strategy discovery: economic viability is a hard precondition, not a post-hoc optimization.
    try:
        audit=CostAwareStrategyAudit(postgres,get_module_logger("cost-aware-strategy-audit"),settings.collector_pairs,exchange=settings.exchange_name,pullback_bars=3,max_bars_per_pair=3200,notional_eur=PER_TRADE_EUR,buy_fee_rate=settings.binance_buy_fee_rate,sell_fee_rate=settings.binance_sell_fee_rate,spread_rate=settings.binance_spread_rate,slippage_rate=settings.binance_slippage_rate)
        summary=audit.run()
        if not summary.get("skipped"):
            message=audit.format_report(summary)
            if message:event_bus.publish(Event(EventType.REPORT_READY,{"message":message,"trusted_html":True,"report_type":"COST_AWARE_STRATEGY_AUDIT"}))
            logger.info("COST_AWARE_STRATEGY_AUDIT_REPORT_SENT rows=%s hold=%s",summary.get("rows",0),summary.get("hold",0))
        else:logger.info("COST_AWARE_STRATEGY_AUDIT_SKIP already_completed=true")
    except Exception:logger.exception("COST_AWARE_STRATEGY_AUDIT_FATAL_GUARD action=CONTINUE_RUNTIME")
