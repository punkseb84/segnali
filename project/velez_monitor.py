...
marked EXPIRED during a runtime migration before their real market outcome was
persisted. Reconciliation is deliberately fail-safe: an audit problem is logged
but can never crash the live PAPER runtime.
"""
...
class Velez15mMonitor(RelativeStrengthMonitor):
    ...
    def _row_to_signal(self, row: Any, *, archived: bool = False) -> dict[str, Any]:
        signal = {
            ...
            "telegram_sent_at": row[9], "exchange": str(row[10] or "Kraken"),
            "quantity": float(row[11] or 0.0),
            "estimated_buy_fee_eur": float(row[12] or 0.0),
            "state": self._decode_state(row[13]),
        }
        if archived:
            signal["archived_at"] = row[14]
            signal["previous_resolution"] = str(row[15] or "")
        return signal
    ...
    def _migration_expired_signals(self) -> list[dict[str, Any]]:
        """Load legacy Velez records retired by deploys and still lacking a true outcome.

        Do not depend on one exact migration-resolution string: older deploys used
        several labels. Restricting to Velez strategy names plus EXPIRED status is
        safer and still avoids touching unrelated strategies.
        """
        self.logger.info("VELEZ_MIGRATION_QUERY_START")
        rows = self.postgres.fetch_all(
            """SELECT id, strategy, pair, timeframe, entry, stop_loss, take_profit,
                      reference_candle_time, created_at, telegram_sent_at,
                      COALESCE(exchange, 'Kraken'), quantity,
               ...
               WHERE status='EXPIRED'
                 AND telegram_sent_at IS NOT NULL
                 AND LOWER(COALESCE(strategy,'')) LIKE '%velez%'
                 AND outcome_price IS NULL
               ORDER BY created_at ASC
               LIMIT 500"""
        )
        self.logger.info("VELEZ_MIGRATION_QUERY_DONE rows=%s", len(rows))
        return [self._row_to_signal(row, archived=True) for row in rows]
    ...
    def _legacy_first_terminal_event(self, signal: dict[str, Any]) -> tuple[str, float, dict[str, Any]] | None:
        ...
        if state.get("fixed_target_active") is False or bool(state.get("stored_target_accounting_only")) or "MODE2" in runtime:
            target_active = False

        for ts, open_, high_, low_, close_ in rows:
            row = {"timestamp": self._utc(ts), "open": float(open_), "high": float(high_),
                   "low": float(low_), "close": float(close_)}
            high, low = row["high"], row["low"]
            stop_hit = low <= stop if direction == "LONG" else high >= stop
            target_hit = target_active and (high >= target if direction == "LONG" else low <= target)
    ...
    def _close_expired_reconciled(self, signal: dict[str, Any], price: float, row: dict[str, Any], resolution: str) -> bool:
        ...
        patch = {
            "gross_pnl_eur": pnl["gross_pnl_eur"], "entry_fee_eur": pnl["entry_fee_eur"],
            "exit_fee_eur": pnl["exit_fee_eur"], "spread_cost_eur": pnl["spread_cost_eur"],
            "slippage_cost_eur": pnl["slippage_cost_eur"], "total_costs_eur": pnl["total_costs_eur"],
            "migration_reconciled": True, "migration_previous_resolution": signal.get("previous_resolution"),
        }
        self.postgres.execute(
            ...
            (status, price, float(row["open"]), float(row["high"]), float(row["low"]), float(row["close"]),
             row["timestamp"], row["timestamp"], resolution == "VELEZ_LEGACY_AMBIGUOUS_BOTH_HIT",
        ...
        payload = {**signal, **patch, "signal_id": signal["id"],
                   "direction": signal["state"].get("direction", "LONG"), "outcome": status,
                   "outcome_price": price, "close_price": float(row["close"]),
    ...
    def reconcile_migration_expired_outcomes(self) -> int:
        """Fail-safe repair of Velez signals archived by old deploys."""
        self.logger.info("VELEZ_MIGRATION_OUTCOME_AUDIT_START")
        try:
            signals = self._migration_expired_signals()
        except Exception:
            self.logger.exception("VELEZ_MIGRATION_OUTCOME_QUERY_FAILED action=CONTINUE_RUNTIME")
            return 0
        repaired = 0
        self.logger.info("VELEZ_MIGRATION_OUTCOME_AUDIT expired=%s ids=%s", len(signals), [s["id"] for s in signals])
        for signal in signals:
            try:
                event = self._legacy_first_terminal_event(signal)
                if event is None:
                    self.logger.info("VELEZ_MIGRATION_OUTCOME_NONE id=%s pair=%s archived_at=%s", signal["id"], signal["pair"], signal.get("archived_at"))
                    continue
                resolution, price, row = event
                if self._close_expired_reconciled(signal, price, row, resolution):
                    repaired += 1
            except Exception:
                self.logger.exception("VELEZ_MIGRATION_OUTCOME_AUDIT_FAILED id=%s pair=%s action=CONTINUE", signal.get("id"), signal.get("pair"))
        self.logger.info("VELEZ_MIGRATION_OUTCOME_AUDIT_DONE expired=%s repaired=%s", len(signals), repaired)
        return repaired
    ...
    def reconcile_all_active_hard_stops(self) -> int:
        try:
            ...
            self.logger.exception("VELEZ_STARTUP_STOP_QUERY_FAILED action=CONTINUE_RUNTIME")
            return 0
        closed = 0
        ...
        for signal in active:
            try:
                ...
                self.logger.exception("VELEZ_STARTUP_STOP_AUDIT_FAILED id=%s pair=%s action=CONTINUE", signal.get("id"), signal.get("pair"))
    ...
    def _persist_protection(self, signal: dict[str, Any], trigger: float, protected_stop: float) -> None:
        ...
        if bool(state.get("profit_protection_armed")):
            return
        patch = {"profit_protection_armed": True, "profit_protection_trigger_price": trigger,
        ...
        self.postgres.execute(
            """UPDATE signals.generated_signals SET score_breakdown=COALESCE(score_breakdown,'{}'::jsonb)||%s::jsonb
               WHERE id=%s AND status IN ('NEW','OPEN')""", (json.dumps(patch), signal["id"]),
        )
        state.update(patch)
        direction = str(state.get("direction", "LONG"))
        message = (
            "🛡️ <b>PROTEZIONE PROFITTO ATTIVATA · PAPER</b>\n"
            f"🆔 <code>#{signal['id']}</code>\n<b>{signal['pair']}</b> · <b>{direction}</b> · 15m\n\n"
            f"Raggiunto: <b>+1,5R</b> ({trigger:.8f})\nNuovo stop protetto: <b>+1R</b> ({protected_stop:.8f})\n\n"
            "La posizione resta aperta. Uscita su stop protetto, chiusura 15m contro EMA20 o limite 12 ore."
        )
        self.event_bus.publish(Event(EventType.REPORT_READY, {"message": message, "trusted_html": True,
            "signal_id": signal["id"], "pair": signal["pair"], "timeframe": "15m",
    ...
    def _check_live_levels(self, signal: dict[str, Any]) -> bool:
        ...
        if stop_hit:
            return self._close(signal, active_stop, self._live_row(price), "VELEZ_PROTECTED_STOP" if armed else "VELEZ_HARD_STOP")
    ...
    def _check(self, signal: dict[str, Any]) -> bool:
        ...
        _, hard_stop, trigger, protected_stop = self._levels(signal, direction)
        armed = bool(signal["state"].get("profit_protection_armed"))
        max_hold = max(1, min(int(signal["state"].get("max_hold_candles") or self.MAX_HOLD_CANDLES), self.MAX_HOLD_CANDLES))
