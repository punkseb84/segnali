"""Daily-limited wrapper for the only active Donchian strategy."""
...
from project.donchian_scanner import DonchianBreakoutScanner, RUNTIME_VERSION
from project.strategy_engine.service import GeneratedSignal


class DailyLimitedDonchianScanner(DonchianBreakoutScanner):
    ...
    def evaluate(self) -> GeneratedSignal | None:
        ...
        if remaining <= 0:
            self._last_cycle_summary = {
                ...
                "rejections": {"DONCHIAN_DAILY_LIMIT": len(self.pairs)},
            }
            self.logger.info(
                "DONCHIAN_DAILY_LIMIT reached today=%s/%s",
                sent_before,
        ...
        for pair in self.pairs:
            try:
                ...
                self.logger.exception("DONCHIAN pair_failed pair=%s", pair)
        ...
        for candidate in candidates:
            ...
            self.logger.info(
                "DONCHIAN_SIGNAL_SENT id=%s pair=%s score=%.1f entry=%.8f stop=%.8f target=%.8f net_rr=%.3f",
                signal_id,
                signal.pair,
                signal.score,
                signal.entry,
        ...
        self.logger.info(
            "DONCHIAN_SUMMARY pairs=%s qualified=%s published=%s today=%s/%s best=%s rejections=%s",
            len(self.pairs),
            len(candidates),
            len(published),
            total_today,
