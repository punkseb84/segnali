"""Version-correct report for the validated V5.1.3 scanner."""
...
class ValidatedReportStrategyScanner(ValidatedLiveStrategyScannerV513):
    def publish_daily_signal_report(self) -> bool:
        ...
        near_lines: list[str] = []
        for item in (summary.get("near_setups") or [])[:5]:
        ...
        if completed:
            performance_text = (
                f"• Operazioni non ambigue chiuse: <b>{completed}</b> · "
            ...
            performance_text = (
                "• Nessuna operazione non ambigua della V5.1.3 ancora chiusa.\n"
        ...
        message = (
            "📊 <b>REPORT SCANNER LONG V5.1.3</b>\n\n"
            "🔎 <b>Ultimo ciclo</b>\n"
            ...
            f"• Segnali V5.1.3 consegnati ultime {self.daily_signal_report_hours}h: <b>{recent_count}</b>\n"
            f"• Stop consecutivi non ambigui V5.1.3: <b>{int(summary.get('loss_streak', 0) or 0)}</b>\n"
            f"• Modalità prudente: <b>{'ATTIVA' if summary.get('loss_guard_active') else 'non attiva'}</b>\n"
            "• Relative Strength: <b>modalità rigorosa · R/R netto minimo 1,45</b>\n"
            ...
            + "\n\n🛡 <b>Stato strategie</b>\n"
            + "\n".join(state_lines)
            + "\n\n📈 <b>Risultati forward V5.1.3</b>\n"
            + performance_text
            + "\n\nLe candele che toccano TP e SL nello stesso intervallo sono escluse dalle statistiche."
        )
        ...
        self.logger.info(
            "REPORT_V5_1_3_SENT strategies=%s recent_delivered=%s loss_streak=%s guard=%s",
